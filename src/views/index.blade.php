...
@section('laravel-login::content')
    <form class="form-signin" role="form" action="{{route('authenticate')}}" method="post" accept-charset="utf-8">
        ...
        <input type="email" id="inputEmail" name="email" class="form-control auth-group" placeholder="{{Lang::get('laravel-login::labels.email')}}" required autofocus>
        <label for="inputPassword" class="sr-only">{{Lang::get('laravel-login::labels.password')}}</label>
        <input type="password" id="inputPassword" name="password" class="form-control auth-group" placeholder="{{Lang::get('laravel-login::labels.password')}}" required>
        <div class="checkbox">
            <label>
